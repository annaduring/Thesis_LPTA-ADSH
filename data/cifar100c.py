...
def load_data(root, num_query, num_train, batch_size, num_workers):
    ...
    """
    CIFAR100c.init(root, num_query, num_train)
    query_dataset = CIFAR100c('query', transform=query_transform(), target_transform=Onehot())
    train_dataset = CIFAR100c('train', transform=train_transform(), target_transform=None)
    retrieval_dataset = CIFAR100c('database', transform=query_transform(), target_transform=Onehot())
...
class CIFAR100c(Dataset):
    ...
    def init(root, num_query, num_train):
        ...
        query_per_class = num_query // 20
        train_per_class = num_train // 20

        # Permutate index (range 0 - 600 per class)
        perm_index = np.random.permutation(data.shape[0] // 20)

        query_index = perm_index[:query_per_class]
        train_index = perm_index[query_per_class: query_per_class + train_per_class]

        query_index = np.tile(query_index, 20)
        train_index = np.tile(train_index, 20)
        inc_index = np.array([i * (data.shape[0] // 20) for i in range(20)])
        ...
        # Split data, targets
        CIFAR100c.QUERY_IMG = data[query_index, :]
        CIFAR100c.QUERY_TARGET = targets[query_index]
        CIFAR100c.TRAIN_IMG = data[train_index, :]
        CIFAR100c.TRAIN_TARGET = targets[train_index]
        CIFAR100c.RETRIEVAL_IMG = data[retrieval_index, :]
        CIFAR100c.RETRIEVAL_TARGET = targets[retrieval_index]
    ...
    def __init__(self, mode='train',
        ...
        if mode == 'train':
            self.data = CIFAR100c.TRAIN_IMG
            self.targets = CIFAR100c.TRAIN_TARGET
        elif mode == 'query':
            self.data = CIFAR100c.QUERY_IMG
            self.targets = CIFAR100c.QUERY_TARGET
        else:
            self.data = CIFAR100c.RETRIEVAL_IMG
            self.targets = CIFAR100c.RETRIEVAL_TARGET

        self.onehot_targets = encode_onehot(self.targets, 20)
