...
import data.cifar100c as cifar100c
import data.cifar100f as cifar100f
...
def load_data(dataset, root, num_query, num_train, batch_size, num_workers):
    ...
    elif dataset == 'cifar-100c':
        query_dataloader, train_dataloader, retrieval_dataloader = cifar100c.load_data(root,
                                                                                     num_query,
                                                                                     num_train,
                                                                                     batch_size,
                                                                                     num_workers,
                                                                                     )        
    elif dataset == 'cifar-100f':
        query_dataloader, train_dataloader, retrieval_dataloader = cifar100f.load_data(root,
                                                                                     num_query,
                                                                                     num_train,
                                                                                     batch_size,
                                                                                     num_workers,
...
def wrap_data(data, targets, batch_size, root, dataset):
    ...
    class MyDataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            elif self.dataset == 'cifar-100c':
                img = Image.fromarray(self.data[index])
                if self.transform is not None:
                    img = self.transform(img) 
            elif self.dataset == 'cifar-100f':
                img = Image.fromarray(self.data[index])
                if self.transform is not None:
                    img = self.transform(img) 
            else:
