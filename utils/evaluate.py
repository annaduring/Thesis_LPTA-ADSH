...
def pr_curve(query_code, database_code, query_targets, database_targets, device):
    """
    P-R curve.
    ...
    for i in range(num_query):
        gnd = (query_targets[i].unsqueeze(0).mm(database_targets.t()) > 0).float().squeeze()
        tsum = torch.sum(gnd)
        if tsum == 0:
            continue
        hamm = 0.5 * (database_code.shape[1] - query_code[i, :] @ database_code.t())
