...
import numpy as np
import time
import models.model1 as attention
...
def train(
    ...
    model = attention.load_model(code_length).to(device)
    ...
    #torch.save(P.cpu(), os.path.join('checkpoints', 'Precision.t'))
    #torch.save(R.cpu(), os.path.join('checkpoints', 'Recall.t'))
